...
it('prepares an isolated real-revision replay without editing the completed batch or inventing observation evidence', async () => {
  ...
  try {
    ...
    const evidenceCache = { schema: 'upstream-radar.dsh-environment-evidence-cache/v1alpha1', collectorFingerprint: 'f'.repeat(64), entries: [] }
    await writeFile(join(review, 'recommendations.json.evidence.json'), JSON.stringify(evidenceCache))
    ...
    assert.deepEqual(await json(join(output, 'review/recommendations.json.evidence.json')), evidenceCache, 'unchanged repository collections survive selective replay')
    assert.deepEqual(await json(join(output, 'baseline/recommendations.json.evidence.json')), evidenceCache)
