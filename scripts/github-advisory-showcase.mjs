...
assert.equal(firstVulnerabilities[0]?.advisory.conflicts?.some(conflict => conflict.field === 'fixed-versions'), true)
...
console.log(`  source conflicts: ${firstVulnerabilities[0]?.advisory.conflicts?.map(conflict => conflict.field).join(', ')}`)
