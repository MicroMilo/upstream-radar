...
record remains pending and can be retried with `--retry-pending`. The report
still includes the pending task's repository, commit range, source/published
versions, changed runtime files, and added dependency edges, so the result is
useful even while the model is unavailable. A source manifest version that
differs from the observed npm version is called out as release-provenance drift
...
The latest live replay queried 513 exact packages with no source errors and found two actionable DSH `rc.6` → `rc.7` compatibility events. The candidate dependency graph was unavailable, so Radar kept both updates at “needs analysis” instead of recommending them as safe; the report includes the affected plugin and exact candidate for each event.
As a follow-up, the same run now loads the exact published artifacts in disposable DSH profiles for both versions: both plugins pass `rc.6` and `rc.7` at the bundle-load boundary. This resolves “can DSH load this bundle?” without pretending that an unavailable candidate dependency graph is a security approval; see the [human-readable follow-up](examples/dsh/reports/dsh-rc6-rc7-compatibility-follow-up-2026-08-18.md).
...
pnpm dlx --package=upstream-radar@latest upstream-radar scan https://github.com/owner/repository --fail-on never
