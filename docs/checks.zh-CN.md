...
| DSH 宿主覆盖 | 缺失的 peer 是可选平台包，还是 DSH/Cordis 宿主依赖没有暴露准确版本 | 读取 `peerDependenciesMeta.optional`；对未解析的 DSH/Cordis peer 单独计数；没有宿主版本时不猜测、不查询 | `optional absent` 或 `DSH host dependency not observed`；覆盖仍为 `incomplete` |
