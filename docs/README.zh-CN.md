...
```bash
npx --yes upstream-radar@0.44.0 inspect dsh-feishu-bot@0.15.8 --deep
```
...
```bash
npx --yes upstream-radar@0.44.0 graph reverse ./plugin-reports \
  --output ./reverse-dependency-index.json
npx --yes upstream-radar@0.44.0 observe ./targets.yml \
...
```bash
npx --yes upstream-radar@0.44.0 observe \
...
pnpm dlx --package=upstream-radar@0.44.0 upstream-radar graph pnpm-lock pnpm-lock.yaml --json
```
...
jobs:
  dependency-radar:
    ...
    steps:
      ...
      - uses: MicroMilo/upstream-radar@v0.44.0
        with:
          fail-on: high
...
```bash
pnpm dlx --package=upstream-radar@0.44.0 upstream-radar review dsh-plugin \
...
```bash
pnpm dlx --package=upstream-radar@0.44.0 upstream-radar benchmark compatibility
```
...
pnpm dlx --package=upstream-radar@0.44.0 upstream-radar probe dsh-load \
...
```bash
pnpm dlx --package=upstream-radar@0.44.0 upstream-radar probe dsh-matrix \
  ./dsh-plugin-1.2.3.tgz \
...
```bash
pnpm dlx --package=upstream-radar@0.44.0 upstream-radar radar check \
