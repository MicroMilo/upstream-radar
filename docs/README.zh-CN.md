...
```bash
npx --yes upstream-radar@0.43.4 inspect dsh-feishu-bot@0.15.8 --deep
```
...
pnpm dlx --package=upstream-radar@0.43.4 upstream-radar graph pnpm-lock pnpm-lock.yaml --json
```
...
jobs:
  dependency-radar:
    ...
    steps:
      ...
      - uses: MicroMilo/upstream-radar@v0.43.4
        with:
          fail-on: high
...
```bash
pnpm dlx --package=upstream-radar@0.43.4 upstream-radar review dsh-plugin \
...
```bash
pnpm dlx --package=upstream-radar@0.43.4 upstream-radar benchmark compatibility
```
...
pnpm dlx --package=upstream-radar@0.43.4 upstream-radar probe dsh-load \
...
```bash
pnpm dlx --package=upstream-radar@0.43.4 upstream-radar probe dsh-matrix \
  ./dsh-plugin-1.2.3.tgz \
...
```bash
pnpm dlx --package=upstream-radar@0.43.4 upstream-radar radar check \
