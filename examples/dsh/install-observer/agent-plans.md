...
|  |  |  |  | The plugin's peer dependency @deepseek-ai/dsh-client-ui-primitives is missing in the runtime graph, but the plugin declares a web platform and requires UI primitives that are not provided by the headless DSH profile. The headless environment cannot satisfy this peer contract, and no build packages are required or approved. Retrying headless would not resolve the missing peer dependency. |
...
|  |  |  |  | The plugin's peer dependencies require @deepseek-ai/dsh-agent@0.1.0-rc.7 and other rc.7 packages, but the host DSH provides 0.1.1-rc.2. The observed result is peer-contract-incompatible with no build packages required. The mismatch is a version contract issue that cannot be resolved by approving builds; it requires updating the plugin's peer dependency ranges or using a compatible DSH version, which is outside the headless execution plane. |
| `dsh-codex-subscription-node22` | `peer-contract-incompatible` | `stop-headless` | `headless-contract` | none |
|  |  |  |  | The plugin requires a web client platform and injects client UI modules, but the headless DSH profile cannot provide the web runtime. The missing peer dependencies are UI primitives and slots, which are part of the web client contract, not build-time packages. No build approval can resolve this. |
...
|  |  |  |  | The plugin installed and loaded, but the DSH runtime did not resolve @deepseek-ai/dsh-client-ui-slots, which the plugin imports at runtime. This is a runtime contract mismatch, not a build approval issue. No build packages are required, so a retry cannot fix the missing peer resolution. |
...
|  |  |  |  | The runtime graph has 2 unresolved peer dependencies from dsh-better-sidebar (@deepseek-ai/dsh-client-ui-primitives and @deepseek-ai/dsh-client-ui-slots) that are not build packages but peer dependencies requiring specific DSH packages. These cannot be resolved by approving build packages; they require a different execution plane (e.g., a full DSH profile with those packages available). |
| `dsh-skills-manager-node22` | `peer-contract-incompatible` | `stop-headless` | `headless-contract` | none |
|  |  |  |  | The plugin loaded but the DSH runtime did not resolve the peer dependency @deepseek-ai/dsh-client-ui-slots, which is required by the plugin's manifest. The headless profile cannot provide this missing peer, and no build packages are required or approved. Retrying headless would not change the outcome. |
...
|  |  |  |  | The plugin installed and loaded, but the runtime graph has 3 unresolved peer dependencies on @deepseek-ai/dsh-client-ui-primitives and @deepseek-ai/dsh-client-ui-slots. These are SDK peer contracts that cannot be satisfied by approving build scripts; they require a different execution plane (e.g., a Web/TUI environment with proper SDK resolution). No build packages are required by the latest retry, so retry-headless is not justified. |
